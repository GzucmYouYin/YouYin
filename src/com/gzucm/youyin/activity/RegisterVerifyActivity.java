...
public class RegisterVerifyActivity extends BaseWapperActivity{
	...
	private String mareaNum; //获取从注册界面传过来的国家区号
	...
	public void getVoiceVerifyCode() {
		final BaseDialog baseDialog = new BaseDialog(RegisterVerifyActivity.this, "语音验证码", "我们将以电话形式通知到你，请注意接听", "好");
		baseDialog.show();
	...
	protected void onHeadRightButton(View v) {
		...
		if (NetUtil.hasNetwork(RegisterVerifyActivity.this)) {
			//验证码验证是否正确，正确跳转到设置密码，否则重新输入验证码
			VerifySMSCode();

		} else {
			Toast.makeText(RegisterVerifyActivity.this, getString(R.string.net_error), Toast.LENGTH_SHORT).show();  //网络未连接
		}
	...
	private void VerifySMSCode(){
		...
		if (code.length() != 4) {
			Toast.makeText(RegisterVerifyActivity.this, "验证码输入有误", Toast.LENGTH_LONG).show();
			return;
		}
	...
	public void backOrWait() {
		final BaseDialog baseDialog = new BaseDialog(RegisterVerifyActivity.this, "提示","确定取消并返回？", "返回", "取消");
		baseDialog.show();
		baseDialog.setClicklistener(new BaseDialog.ClickListenerInterface() {
	...
	private void afterSubmit(final int result, final Object data) {
		if (result == SMSSDK.RESULT_COMPLETE) {
			Intent intent = new Intent(RegisterVerifyActivity.this, SetpasswordActivity.class);
			intent.putExtra("phone",mphone);
			startActivity(intent);
		} else {
			((Throwable) data).printStackTrace();
			// 验证码不正确
			Toast.makeText(RegisterVerifyActivity.this, "验证码不正确", Toast.LENGTH_SHORT).show();
	...
	private void afterGet(final int result, final Object data) {
		if (result == SMSSDK.RESULT_COMPLETE) {
			...
			((Throwable) data).printStackTrace();
			// 验证码发送错误
			Toast.makeText(RegisterVerifyActivity.this, "验证码发送出错，请重试", Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void afterGetVoice(final int result, final Object data) {
		...
		if (result == SMSSDK.RESULT_COMPLETE) {
			...
			((Throwable) data).printStackTrace();
			// 语音验证码发送错误
			Toast.makeText(RegisterVerifyActivity.this, "语音验证码发送出错，请重试", Toast.LENGTH_SHORT).show();
		}
	}
